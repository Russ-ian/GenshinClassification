...
class tagsRouter extends BaseRouter {
    init () {
        ...
        this.post ( '/', multer.single( 'image' ), async function( req, res, next ) {
            const { tagName, tagDescription, imagePath } = { ...req.body, imagePath: req.file?.path }; // Get the data

            if ( !tagName || !tagDescription ) return res.sendError( 'Name and Description is required to create' ) // Check if it exists

            const response = await tagsManager.createTag( tagName, imagePath, tagDescription )
